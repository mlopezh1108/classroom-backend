...
class CourseControllerTest {
    ...
    void shouldCreateCourse() throws Exception {
        CreateCourseRequest request = new CreateCourseRequest("MATH101", "TEA-1", "GRP-1", "SUB-1", "PER-1", CourseStatus.OPEN);
        Course course = new Course();
        CourseResponse response = new CourseResponse("course-1", "MATH101", "TEA-1", "GRP-1", "SUB-1", "PER-1", CourseStatus.OPEN);
    ...
    void shouldListTeacherCourses() throws Exception {
        CourseResponse response = new CourseResponse("course-1", "MATH101", "TEA-1", "GRP-1", "SUB-1", "PER-1", CourseStatus.OPEN);
        when(listTeacherCoursesUseCase.execute("TEA-1")).thenReturn(List.of(new Course()));
        when(courseMapper.toResponse(any(Course.class))).thenReturn(response);
