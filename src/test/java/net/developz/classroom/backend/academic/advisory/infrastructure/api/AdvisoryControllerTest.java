...
class AdvisoryControllerTest {
    ...
    void shouldScheduleAdvisory() throws Exception {
        ...
        AdvisoryDTO dto = new AdvisoryDTO("adv-1", "enroll-1", AdvisoryStatus.SCHEDULED, LocalDate.now(), LocalTime.now(), null);

        when(mapper.toEntity(any(ScheduleAdvisoryRequest.class))).thenReturn(advisory);
    ...
    void shouldRecordSession() throws Exception {
        ...
        Advisory advisory = new Advisory();
        AdvisoryDTO dto = new AdvisoryDTO("adv-1", "enroll-1", AdvisoryStatus.COMPLETED, LocalDate.now(), LocalTime.now(), "Notes");
